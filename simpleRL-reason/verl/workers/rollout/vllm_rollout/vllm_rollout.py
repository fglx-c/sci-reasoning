    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        if "mistral" in config.model_path.lower():
            ...
            if "7b" in config.model_path.lower():
                ...
                kwargs['stop_token_ids'] = [14582, 16141, 31198] # Question, Answer, Problem
        elif "openreasoning-nemotron" in config.model_path.lower():
            # OpenReasoning-Nemotron models are based on Qwen2.5, use corresponding stop tokens
            if "1.5b" in config.model_path.lower():
                kwargs['stop_token_ids'] = [14582, 16141, 31198] # Question, Answer, Problem (same as Qwen2.5-1.5B)
            elif "7b" in config.model_path.lower():
                kwargs['stop_token_ids'] = [151645, 151643, 872,77091, 1474, 71703, 151644, 8948] # same as Qwen2.5-7B
            elif "14b" in config.model_path.lower() or "32b" in config.model_path.lower():
                kwargs['stop_token_ids'] = [151645, 151643, 872,77091, 1474, 71703, 151644, 8948] # use Qwen2.5-7B tokens for larger models
        else:
