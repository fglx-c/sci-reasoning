...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Skip validation only for non-verifier/non-pairwise model-based rewards
            if (self.config.reward_model.enable and 
                test_batch[0].non_tensor_batch['reward_model']['style'] == 'model' and
                self.config.reward_model.strategy not in ['verifier', 'pairwise']):
                return {}
            ...
            # For verifier and pairwise strategies, compute rm_scores before calling reward function
            if (self.config.reward_model.enable and 
                self.config.reward_model.strategy in ['verifier', 'pairwise']):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # Check if verifier already provided scores, if so skip function-based rewards
                        if 'rm_scores' in batch.batch.keys():
                            # Use verifier scores directly
                            batch.batch['token_level_scores'] = batch.batch['rm_scores']
                            # Set dummy correctness for compatibility (not used in verifier-only mode)
                            batch.batch['correctness'] = torch.zeros(len(batch), dtype=torch.float32)
                        else:
                            # Fallback to function-based rewards when no verifier
