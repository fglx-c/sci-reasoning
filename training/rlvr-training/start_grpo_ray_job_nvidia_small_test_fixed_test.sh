...
export RAY_ADDRESS="${MACHINE_IP}:6379"  # For ray status
...
REWARD_STRATEGY="pairwise"  # Default strategy: pairwise 
...
if [[ "$REWARD_STRATEGY" == "pairwise" && "$ROLLOUT_N" == "1" ]]; then
    echo "Pairwise strategy requires multiple samples for comparison. Setting rollout_n=2"
    ROLLOUT_N=2
fi
...
# Check Ray status and start if needed
echo "Checking Ray cluster status..."
export RAY_ADDRESS="${MACHINE_IP}:6379"
if ! ray status > /dev/null 2>&1; then
    echo "Ray cluster not running. Starting Ray..."
    ray start --head --node-ip-address ${MACHINE_IP} --num-gpus 4 --disable-usage-stats
    sleep 10
else
    echo "Ray cluster is already running."
fi

# Wait a moment for Ray to be fully ready
echo "Waiting for Ray cluster to be fully ready..."
sleep 5

# Skip dashboard check due to curl/OpenSSL issues on Holly system
# The training works fine even if dashboard check fails
